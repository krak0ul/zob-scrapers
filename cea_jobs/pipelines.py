...
Pipelines
────────
1. DuplicateFilterPipeline  — drops URL duplicates within a run
2. CsvExportPipeline        — timestamped CSV in output/
3. JsonExportPipeline       — timestamped JSONL in output/
"""
...
class CsvExportPipeline:
    def open_spider(self, spider):
        _ensure_dir()
        self.job_path = os.path.join(OUTPUT_DIR, f"cea_jobs_{TIMESTAMP}.csv")
        self.detail_path = os.path.join(OUTPUT_DIR, f"cea_detail_{TIMESTAMP}.csv")
...
class JsonExportPipeline:
    def open_spider(self, spider):
        _ensure_dir()
        self.job_path = os.path.join(OUTPUT_DIR, f"cea_jobs_{TIMESTAMP}.jsonl")
        self.detail_path = os.path.join(OUTPUT_DIR, f"cea_detail_{TIMESTAMP}.jsonl")
